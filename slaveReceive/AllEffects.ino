...
void setup()
{
  ...
  //add led array with color correction
...
// this function is called whenever a 'lightStripMode' message
// is received from Adafruit IO. it was attached to
// the lightStripMode feed in the setup() function above.

void lightmode(AdafruitIO_Data *data) {
  Serial.print("received google-assistant.lightstripmode <- ");
  
  String effectRequest = data->value();
  Serial.println(effectRequest);

  digitalWrite(LED_BUILTIN, HIGH);

  if (effectRequest.equalsIgnoreCase("random")) {
    changeEffect(random(0,18));
  } else if (effectRequest.toInt() >= 0 && effectRequest.toInt() <= 18) {
    changeEffect(effectRequest.toInt());
  }
...
void brightness(AdafruitIO_Data *data) {
  Serial.print("received google-assistant.lightstripbrightness <- ");
  ...
  FastLED.setBrightness(2.55*(effectRequest.toInt()));
  delay(500);
  digitalWrite(LED_BUILTIN, LOW);
}
...
void changeEffect(int effect) {
  ...
  Serial.println("Changed effect to:");
